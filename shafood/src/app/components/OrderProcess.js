...
const OrderProcess = ({ chatId, users, currentUserId, onlineUsers }) => {
    ...
    const handleOrderUpdate = (data) => {
        ...
        if (data.type === 'orderer-update') {
            ...
            const ordererName = users.find(u => u.id === data.ordererId)?.name || 'Someone';
            ...
            if (data.ordererId) {
                ...
                toast.info(`${ordererName} is now the orderer`);
        ...
        } else if (data.type === 'status-update' || data.type === 'reset') {
            ...
            const newReceivedCount = data.receivedCount || Object.values(data.userStatuses || {})
            ...
            if (data.type === 'status-update') {
                ...
                if (data.orderStatus === 'completed') {
                    ...
                    toast.success('All orders have been received!');
                ...
                } else if (data.orderStatus === 'ordered') {
                    ...
                    const userName = users.find(u => u.id === data.lastReceivedBy)?.name;
                    ...
                    if (userName) {
                        ...
                        toast.info(`${userName} has marked their order as received`);
    ...
    const updateStatus = async (status) => {
        ...
        if (status === 'received') {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Check if all users have received their orders
                    ...
                    if (allReceived) {
                        ...
                        // If all users have received, update the order status to completed
                        ...
                        toast.success('All users have received their orders. Order completed!');
        ...
        // For other statuses (pending, ordered, completed)
    ...
    const updateOrderStatus = async (status) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // If status is completed, enable the reset button
    ...
    const toggleOrderer = async (userId) => {
        ...
        try {
            ...
            setSelectedOrderer(userId === selectedOrderer ? null : userId);
                ...
                const ordererName = users.find(u => u.id === userId)?.name || 'User';
                ...
                toast.success(userId === selectedOrderer ? `${ordererName} is no longer the orderer.` : `${ordererName} is now the orderer.`);
    ...
    const handleMarkAsReceived = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const allReceived = Object.values(data.userStatuses).every(status => status.received);
                ...
                if (allReceived) {
                    ...
                    // Automatically reset the order process
    ...
    const handleResetOrder = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/chat/${chatId}/reset-order`, {
    ...
    // Add this new function to check if reset should be enabled
    ...
    const isResetEnabled = useCallback(() => {
        ...
        // Enable reset if all users have marked as received
        ...
        const allReceived = Object.values(userStatuses).every(status => status==="received");
        ...
        // Also enable reset if the order status is 'completed' regardless of received status
        ...
        return allReceived || orderStatus === 'completed';
    ...
    // Add this function to check if the "Mark as Ordered" button should be enabled
    ...
    const canMarkAsOrdered = () => {
        ...
        return orderStatus === 'pending' && selectedOrderer === currentUserId;
    ...
    // Add this function to check if the "Mark as Received" button should be enabled
    ...
    const canMarkAsReceived = () => {
        ...
        return orderStatus === 'ordered' && !userStatuses[currentUserId]?.received;
        ...
        <div className="p-4 h-full overflow-y-auto">
            ...
            {orderStatus === 'pending' && !selectedOrderer && (
                ...
                <p className="text-center mt-4 text-orange-600 dark:text-orange-400">
                    ...
                    Please select an orderer to proceed
