...
const ChatPageContent = ({ initialSelectedUsers }) => {
  ...
  const [selectedUsers, setSelectedUsers] = useState(() => {
    ...
    if (session?.user) {
      ...
      const filteredUsers = initialSelectedUsers.filter(user => user.id !== loggedInUser.id);

      

      return [loggedInUser, ...filteredUsers];
  ...
  useEffect(() => {
    ...
    if (session?.user) {
      ...
      if (!isLoggedInUserIncluded) {
        ...
        setSelectedUsers(prevUsers => [loggedInUser, ...prevUsers]);
    ...
    const handleUrlChange = () => {
      ...
      if (usersParam) {
        ...
        try {
          ...
          // Add logged-in user if not present
